...
<img src="./assets/8d17d4ed997300dad29d868b16039850.png" style="zoom:90%;" />

> [!note]
>
> 退化问题（Addressing Degradation Problem）
>
> 在深度神经网络中，随着网络深度的增加，理论上模型的能力应该增强，可以学习更复杂的特征。然而，实际情况是当网络深度增加到一定程度后，模型的性能不仅没有提升，反而开始下降，这种现象被称为退化问题 。退化问题不是过拟合，退化问题即使在训练集上，更深的网络也比更浅的网络表现更差。 这意味着更深的网络训练过程，遇到了困难。

针对这一问题，何恺明等人提出了残差网络残差网络（ResNet）在2015年的ImageNet图像识别挑战赛夺魁，并深刻影响了后来的深度神经网络的设计。

<img src="./assets/08803e5a9b2d589cc873657fd0903fef.png" style="zoom:60%;" />
...
<img src="./assets/4513e8ffd7dd54db4ff54d3c3a542c6c.png" style="zoom:60%;" />
...
<img src="./assets/e7c3b6da09fcc5aea9f514d29eb4deb6.png" style="zoom:65%;" />
...
![](./assets/420d05bfd2718b5870d436f26db2b84d.png)
...
最优参数为`optimizer__weight_decay=0.005`，`lr=0.001`。使用最优参数训练模型
...
模型微调（Fine-tuning）是迁移学习（Transfer Learning）中的一种常用技术，指在一个预训练模型（Pre-trained Model）的基础上，通过在新的数据集上进行额外训练，使其适应新任务的过程。

* 模型微调避免了从头训练数据。
* 模型微调一般是在大量数据集上训练模型，以适应小数据集任务。

这里使用CIFAR100的数据作为预训练数据，在CIFAR10上进行微调。由于同一个模型，同时要适应100分类和10分类任务，修改resnet18的网络结构。将卷积网络结构提取成骨干网络。
...
```python
full_100 = datasets.CIFAR100(root="./data", train=True, download=True)
test_100 = datasets.CIFAR100(root="./data", train=False, download=True)
full_data_100 = PackDataset(full_100, transform=trans_aug())
test_data_100 = PackDataset(test_100, transform=transforms.ToTensor())
```
...
```python
epochs = 45
calls = control_callbacks(epochs, check_dir='./data/res-backbone-checkpoints', show_bar=False)
net = NeuralNetClassifier(
    resnet18,
    criterion=nn.CrossEntropyLoss,
    optimizer=torch.optim.Adam,
    ...
    batch_size=2048,
    max_epochs=epochs,
    train_split=predefined_split(test_data_100),
    device='cuda' if torch.cuda.is_available() else 'cpu',
    callbacks=calls,
    classes=list(range(10)),
    iterator_train__num_workers=8,
...
net.fit(full_data_100, y=None)
```

保存骨干网络的训练结果

```python
torch.save(resnet18.get_backbone_state_dict(), './data/cifar100_backbone.pth')
```

使用CIFAR100的训练参数创建一个10分类模型

```python
backbone_load = ResNet18Backbone()
backbone_load.load_state_dict(torch.load('./data/cifar100_backbone.pth'))
resnet18_finetune = ResNet18Classifier(backbone_load, num_classes=10)
```

冻结卷积层参数，训练分类网络

```python
resnet18_finetune.freeze_backbone(True)
epochs = 45
calls = control_callbacks(epochs, check_dir='./data/res-backbone-checkpoints', show_bar=False)
net = NeuralNetClassifier(
    resnet18_finetune,
    ...
    batch_size=2048,
    max_epochs=epochs,
    train_split=predefined_split(test_data_10),
    device='cuda' if torch.cuda.is_available() else 'cpu',
    callbacks=calls,
    classes=list(range(10)),
    iterator_train__num_workers=8,
...
net.fit(full_data_10, y=None)
```

取消冻结参数，进一步微调卷积网络

```python
resnet18_finetune.freeze_backbone(False)
epochs = 45
calls = control_callbacks(epochs, check_dir='./data/res-backbone-checkpoints', show_bar=False)
net = NeuralNetClassifier(
    resnet18_finetune,
