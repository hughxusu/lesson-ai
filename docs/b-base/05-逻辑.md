...
在线性回归中
$$
\hat{y}=f(x) \Rightarrow \hat{y}= X_b \cdot w
$$
其中$\hat{y}\in \left [ -\infty, + \infty \right ]$ ，为了使结果映射到概率的值域$\left [0, 1\right ]$，存在函数
$$
\hat{p}=\sigma \left( X_b \cdot w \right)
$$
其中
$$
...
所以概率$\hat{p}$可以表示为
$$
\hat{p}=\sigma \left( X_b \cdot w \right)=\frac{1}{1+e^{-(X_b \cdot w)}} \qquad
\hat{y}=\begin{cases}
 1, & \hat{p}\ge 0.5\\
 0, & \hat{p}< 0.5\\
\end{cases}
...
其中
$$
\hat{p}^{(i)}=\sigma \left( X_b^{(i)} \cdot w \right)=\frac{1}{1+e^{-(X_b^{(i)} \cdot w)}}
$$
...
$$
L=-\frac{1}{m}\sum_i^{m}\left (y^{(i)}\log \left(\sigma \left( t \right)\right)+(1-y^{(i)})\log \left(1-\sigma \left( t \right)\right)\right) \quad t=X_b^{(i)} \cdot w
$$
...
\frac{1}{m}\sum_{i=1}^{m}\left(\sigma(X_b^{(i)}\cdot w)-y^{(i)}\right) x^{(i)}_j=
