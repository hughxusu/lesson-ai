...
> [!note]
>
> 优秀的机器学习算法应当具备良好的维度可扩展性，能够接受不同维度特征向量。
...
$$
无论向量的维度如何，一个函数要能作为“距离”，必须满足三条公理：

1. 非负性：$\text{Dist}(\mathbf{x}^{(a)},\mathbf{x}^{(b)}) \ge 0$。
2. 对称性：$\text{Dist}(\mathbf{x}^{(a)},\mathbf{x}^{(b)}) = \text{Dist}(\mathbf{x}^{(b)},\mathbf{x}^{(a)})$。
3. 三角不等式：$\text{Dist}(\mathbf{x}^{(a)},\mathbf{x}^{(b)}) \le \text{Dist}(\mathbf{x}^{(a)},\mathbf{x}^{(b)}) + \text{Dist}(\mathbf{x}^{(a)},\mathbf{x}^{(b)})$

三角不等式确保了高维空间中距离：

* 可传递性（若A离B很近，B离C很近，则A离C绝不可能无限远）。
* 能映射出一个唯一的非负实数，可以严谨地进行大小比较

KNN算法根据距离最近的K个样本进行分类，所以对不同的维度特征都成立。
...
<img src="./assets/Xnip2025-06-03_18-12-33.jpg" style="zoom:85%;" />
...
<img src="./assets/iris-data-show-20260619175604710.png" style="zoom: 75%;" />
