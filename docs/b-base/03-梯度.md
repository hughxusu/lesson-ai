...
\end{bmatrix}
$$
在高维空间中，通过梯度下降值，来调整$w$的值，以便找到损失函数的最小值点。梯度下降的值
$$
-\eta\nabla L
$$
对于估计值$\hat{y}^{(i)}$可以表示为
$$
\hat{y}^{(i)} =\mathbf{x}^{(i)}_b \cdot w
$$
所以损失函数$L$可以表示为
$$
L=\sum_{i=1}^m \left ( y^{(i)}- \mathbf{x}^{(i)}_b \cdot w \right )^2
$$
根据符合函数求导法则
$$
\frac{\partial L}{\partial w_j }=\sum_{i=1}^m 2\left ( y^{(i)}-\mathbf{x}_b^{(i)}\cdot w \right )(-x_j^{(i)})
$$
所以$\nabla L$，可以表示为
$$
\nabla L = 
...
\end{bmatrix} = 
\begin{bmatrix}
\sum_{i=1}^m 2\left ( y^{(i)}-\mathbf{x}_b^{(i)}\cdot w \right )(-1) \\
\sum_{i=1}^m 2\left ( y^{(i)}-\mathbf{x}_b^{(i)}\cdot w \right )(-x_1^{(i)}) \\
\sum_{i=1}^m 2\left ( y^{(i)}-\mathbf{x}_b^{(i)}\cdot w \right )(-x_2^{(i)}) \\
\vdots \\
\sum_{i=1}^m 2\left ( y^{(i)}-\mathbf{x}_b^{(i)}\cdot w \right )(-x_n^{(i)})
\end{bmatrix} = 
2\begin{bmatrix}
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}w - y^{(i)} \right ) \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}w - y^{(i)} \right )x_1^{(i)} \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}w - y^{(i)} \right )x_2^{(i)} \\
\vdots \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}w - y^{(i)} \right )x_n^{(i)}
\end{bmatrix}
$$
...
\frac{2}{m}
\begin{bmatrix}
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right ) \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_1^{(i)} \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_2^{(i)} \\
\vdots \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_n^{(i)}
\end{bmatrix}
$$
...
\frac{2}{m}
\begin{bmatrix}
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_0^{(i)} \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_1^{(i)} \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_2^{(i)} \\
\vdots \\
\sum_{i=1}^m \left ( \mathbf{x}_b^{(i)}\cdot w - y^{(i)} \right )x_n^{(i)}
...
$$
e=\begin{bmatrix}
\mathbf{x}_b^{(1)}\cdot w - y^{(1)} \\
\mathbf{x}_b^{(2)}\cdot w - y^{(2)} \\
\vdots  \\
\mathbf{x}_b^{(m)}\cdot w - y^{(m)}
\end{bmatrix} \tag{2}
$$
则公式 $(1)$ 中第$j$个元素可以表示为
$$
...
=
\begin{bmatrix}
\mathbf{x}_b^{(1)} \cdot w \\
\mathbf{x}_b^{(2)} \cdot w \\
\vdots \\
\mathbf{x}_b^{(m)} \cdot w
\end{bmatrix}
$$
使用上面的式子整理公式 $(2)$ 可得
$$
...
$$
* 其中$w$是未知数，该公式可以计算出任意的$w$，损失函数$L$在该点的梯度。
...
$$
\frac{\partial L}{\partial w_j}\approx\frac{L(w_j+h)-L(w_j-h )}{2h } \tag{5}
$$
其中$h$是一个极小数，比如$10^5$，而$L(w_j+h)$计算公式为
$$
L(w_j+\epsilon)=
\sum_{i=1}^m \left ( y^{(i)}-w_0-w_1x_1^{(i)}-\cdots-(w_j+h)x_2^{(i)}-\cdots-w_nx_n^{(i)} \right )^2
$$
...
假设计算过程中每次只取一个样本则有
$$
\nabla L_i = 2\left(\mathbf{x}_b^{(i)}\right)^T \left( \mathbf{x}_b^{(i)} w - y^{(i)} \right)
$$

* $\left(\mathbf{x}_b^{(i)}\right)^T$是形状为 $(n \times 1)$ 的列向量；
* $\left(\mathbf{x}_b^{(i)} w - y^{(i)} \right)$是标量；
...
* 每次只用单个样本计算梯度，单个样本的梯度方向包含很大的噪声和随机性。导致更新路径不是一条笔直指向圆心的直线，而是不断发生曲折、甚至有时会向远离圆心的偏斜方向走。
