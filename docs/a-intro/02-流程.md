...
[`sklearn.datasets`](https://scikit-learn.org/stable/api/sklearn.datasets.html)用于数据集加载实验。

* `load_xx`用来加载小型测试数据集。
* `fetch_xx`用来下载并加载大的真实数据集
...
鸢尾花数据集由Fisher收集整理，并发表在1936年的经典论文[《多重测量在分类学问题中的应用》](https://onlinelibrary.wiley.com/doi/epdf/10.1111/j.1469-1809.1936.tb02137.x)中。
...
使用`load_iris`函数可以加载鸢尾花数据集，`keys()`包含了数据集的所有属性。
...
\end{bmatrix}
$$
对于$m$个样本，$n$维特征的数据，可以表示$X\in \mathbb{R}^{m\times n}$的矩阵。
...
> 1. 和特征决定了机器学习的上限，而模型和算法只是逼近这个上限而已。
