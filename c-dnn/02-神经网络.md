...
<img src="https://raw.githubusercontent.com/hughxusu/lesson-ai/developing/_images/nn/08.png" style="zoom:65%;" />
...
函数$h(x)$​可以将输入信号的总和转换为输出信号，这个函数称为激活函数（activation function）。激活函数的作用，在于输入值为多少事有输出值。上面的公式可以整理如下
$$
a = b + w_1x_1+w_2x_2 \\
y=h(a)
$$
...
<img src="https://raw.githubusercontent.com/hughxusu/lesson-ai/developing/_images/nn/tanh.png" style="zoom:85%;" />
...
* 信号整合：
  ...
  - 树突接收信号：化学突触释放神经递质，改变细胞膜内外离子浓度（如Na⁺、K⁺、Cl⁻）。

  - 膜电位变化：局部电位（EPSP/IPSP）在细胞体中累积。


* 激活机制：
  * 阈值触发：若膜电位超过阈值（约-55mV），触发“全或无”的动作电位（action potential）。
  * 脉冲编码：信息通过动作电位的频率和时序编码，而非连续值。

特点：离散（脉冲）、动态（时间依赖）、局部异步处理。

二者的差异对比

| **特性**     | **人类神经元**                    | **人工神经元（全连接层）** |
| :----------- | :-------------------------------- | :------------------------- |
| **信号类型** | 离散脉冲（动作电位）              | 连续数值（浮点数）         |
| **信息编码** | 脉冲频率、时序、同步性            | 数值大小（激活值）         |
| **计算方式** | 动态、异步、局部                  | 静态、同步、全局           |
| **能量效率** | 极高（仅在激活时耗能）            | 低（持续计算）             |
| **学习机制** | 突触可塑性（LTP/LTD）、结构可塑性 | 反向传播、权重更新         |
